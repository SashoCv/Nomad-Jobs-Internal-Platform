...
class CandidateController extends Controller
{
    ...
    /**
     * Clean up auto-generated document files for a specific contract.
     * Lightweight endpoint used before document regeneration — no full candidate validation needed.
     */
    public function cleanupContractDocuments(Request $request, $id): JsonResponse
    {
        if (!$this->isStaff()) {
        ...
        try {
            ...
            $this->candidateService->cleanupAutoGeneratedFiles($candidate, $contractId);

            return response()->json(['success' => true, 'message' => 'Documents cleaned up']);
        } catch (\Exception $e) {
            Log::error('Error cleaning up contract documents: ' . $e->getMessage());
            return response()->json(['success' => false, 'message' => 'Failed to clean up documents'], 500);
        }
    }
