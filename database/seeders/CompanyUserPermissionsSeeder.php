...
class CompanyUserPermissionsSeeder extends Seeder
{
    /**
    ...
    {
        // Company User permissions - limited business access
        $allowedPermissions = [
        ...
        // Assign permissions to Company User role (role_id = 3)
        $companyUserRole = Role::find(Role::COMPANY_USER);
        if ($companyUserRole) {
            $companyUserRole->permissions()->sync($permissionIds);
            
            echo "Company User permissions assigned successfully!\n";
            echo "Company User has access to " . count($permissionIds) . " permissions:\n";
            ...
            echo "Company User uses existing role-based logic for other functions.\n";
        } else {
            echo "Company User role not found!\n";
        }
    }
}